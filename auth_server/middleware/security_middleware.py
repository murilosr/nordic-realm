...
from app.usuario import UsuarioRepository
...
class OAuthSecurityMiddleware(AuthenticationBackend):
    ...
    def _process_authorization(
        self, authorization_header: str | None, path_is_public: bool
    ) -> tuple[AuthCredentials, BaseUser]:
        if authorization_header is None:
            self._raise_error_or_return_unauthenticated(
                "Missing Authorization header", path_is_public
            )

        auth_split = authorization_header.split(" ")
        if len(auth_split) != 2 or auth_split[0] != "Bearer":
            self._raise_error_or_return_unauthenticated(
                "Bad Authorization header", path_is_public
            )
        ...
        return AuthCredentials([]), AuthUser(
            id=session.user_id, username=user.full_name()
        )

    def _return_unauthenticated_credentials(self):
        return AuthCredentials([]), UnauthenticatedUser()

    def _raise_error_or_return_unauthenticated(
        self, error_msg: str, path_is_public: bool
    ):
        if path_is_public:
    ...
    async def authenticate(self, conn):
        if conn.scope["type"] == "websocket":
            return self._process_authorization(
                conn.headers.get("Authorization", None), False
            )
        ...
        return self._process_authorization(
            conn.headers.get("Authorization", None), path_is_public
        )
