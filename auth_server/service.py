...
class AuthServerService:
    ...
    def _decode_token(self, encoded_token: str) -> JWTToken:
        return JWTToken(
            **jwt.decode(
                jwt=encoded_token, key=self.APP_SECRET_KEY, algorithms=["HS256"]
            )
        )
    ...
    def authenticate_by_password(
        ...
        auth_user = self.user_auth_provider.authenticate_by_password(
    ...
    def update_session_expiry_dates(
        ...
        user_session.access_token_expiry_dt = datetime.now() + timedelta(
            seconds=access_token_timedelta_seconds
        )
        user_session.refresh_token_expiry_dt = datetime.now() + timedelta(
            seconds=refresh_token_timedelta_seconds
        )
